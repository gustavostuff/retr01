...
/*
 * Layer-2 islands A–E + G + H + I smoke:
 *   D $FE02 / $FE04 latches, E pads, G VRAM, H beam, I BG nametable fetch.
 */
int main(void) {
    R01sBoard board;
    ...
    expect_true(r01s_island_group_count(group) == 8, "8 islands A-E+G+H+I");
    ...
    for (i = 0; i < 8000; i++) {
        ...
        if (saw_latch && saw_vram && saw_vram_read && saw_pad && saw_beam_hblank && saw_beam_line &&
            saw_raster_hit && saw_bg_tile && saw_bg_attr) {
            break;
        }
    }
    ...
    return test_done("test_island_abcdeghi");
}
