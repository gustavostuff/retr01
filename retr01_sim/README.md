...
**Islands A–E + G models, wiring, and layer-2 smoke.** SDL board UI. Architecture: [`docs/08_simulator.md`](../docs/08_simulator.md).
...
Next: island **H** (beam) / **I** (BG fetch), or more `$FExx` latches.
...
| `src/board.c` | **Board recipe A–E + G** — wiring, settle loop, group vtable |
| `src/main.c` | SDL entry: build board + run UI |
| `chips/` | Per-part models (subclass the base entity) |
| `tests/` | Layer-1 unit tests + `test_island_abcdeg` (layer 2) |
