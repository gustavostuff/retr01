...
On startup (and after Ctrl+R reset), the sim must run the bring-up **palette + MAP stream** on the pin-level netlist before the LCD hold lifts. That is ~12k island steps (`r01s_board_catchup_bringup` / the yielding worker in `app.c`), not a soft poke of VRAM.

Until that finishes, the board is mid-stream: buses, map pointer, and sink contents change every step.
...
- Coarser settle, skip idle islands, or a “bring-up mode” that does not eval full video every step until MAP completes.
- Reduces pressure on any of the above; orthogonal to threading.
