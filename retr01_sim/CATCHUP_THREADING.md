...
4. Wall time is on the order of **~10–15 s** (~1 ms/step × ~12k steps on a typical machine).
...
2. That function **does not** step the netlist. It:
   - peeks 32 BG palette bytes from cart flash → `board->active_pal[]`
...
`board_step` invokes settle **several times** per step (around PHI2 / CPU / beam). So PIN does roughly **4×** the wire work of FAST on every settle call. Deep settle exists so decode → CE → DQ can propagate in one half-cycle; one pass is usually enough once the netlist is “warm,” but shallow settle can hide multi-level glue races.
...
Same silicon timing model (341×262), but FAST advances the raster **16× slower per board step**. Under the UI’s ~10 ms step budget you still get video, but fewer dots (and fewer sink plots) per wall-clock frame. PIN burns most of its step time here during catchup and play.
...
- Play: more PHI2/CPU progress per UI frame for the same wall budget (~5–20× less work per step from settle×beam cuts, depending on hot path).
