...
**This doc only covers what changes for Tier B.** Clocks, DAC, AD724, color kit, bring-up basics, and “do not buy cart/CPU/AVR” rules stay as in the Tier A guide.
...
Still **not** required at Tier B: W65C02, AVRs, SRAMs, HC157/573/574, cart parts, PHI2 oscillator.

**Total PLDs now:** 3 (Beam X, Beam Y, Compositor) — matches the locked motherboard count for PLDs.
...
Macrocell pressure (full design): scroll-Y + other registered work can approach **~21 of 30** macrocells — stay honest; prefer a **1-dot registered** color index if fit allows.
...
Do **not** implement real MAP, soft `$7Fxx`, or VRAM select until Tier C / CPU.
...
Only wire what the Compositor JEDEC actually uses for **index** and **priority**. Document unused I/O in the JEDEC comments so a later full fuse map can reclaim pins.
...
- Force `INDEX = 0` in HBlank/VBlank if you want a quiet DAC during sync (optional).
...
## 5. Do / don’t (Tier B only)

### Do

- Program **all three** PLDs before power-up; keep Beam X/Y JEDECs that already worked in Tier A.  
- Move **only** index generation into the Compositor; do not rewrite sync from scratch unless needed.  
- Prove priority with a **visible** test: e.g. sprite-colored box over bars, with BG1 “holes” (index 0) showing BG0 bands.  
- Scope `INDEX[5:0]` (or PROM A pins) while changing patterns — confirm the Compositor, not only the TV.  
- Keep analog path (PROM → DAC → AD724) unchanged from Tier A.

### Don’t

- Don’t hang a partial CPU bus “for later” on Compositor pins without Hi-Z rules — causes fights when you add silicon.  
- Don’t burn macrocells on full MAP/`SEL_SOFT*` decode yet; stub them.  
- Don’t route HSYNC/VSYNC/CSYNC through the Compositor unless your JEDEC intentionally does; design keeps beam timing on Beam X/Y.  
- Don’t add HC157 / VRAM / S1 until Tier A+B index path is boringly stable.  
- Don’t treat Tier B as “almost full video” — field SRAM and HBlank BG0 fill are still Tier C.
...
**Next (Tier C):** MCU-S1 + field SRAM (+ HC573 if using AD multiplex), VBlank pattern fill, optional BG0 line buffer — still no cart/CPU required for a pure video stress test.

---

*Retr01 Tier B video lab — Compositor-only delta over Tier A.*
