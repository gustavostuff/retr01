...
**Goal:** Get a stable composite (or RGB) picture on a TV/monitor using only the video path.  
**No cart, no CPU, no AVRs, no controllers, no audio, no saves.**

This is the smallest setup that still exercises Retr01’s real color path:
...
Design references (repo docs): `general_docs/hardware.md`, `general_docs/video-graphics.md`, `ic_behavior/AD724.md`, `ic_behavior/ATF22V10.md`, `ic_behavior/AT27C256R.md`.
...
Optional third ATF22V10 (Compositor) is **not** needed for Tier A if you generate the 6-bit index another way (see §5).
...
- Optional: header for raw RGB + sync (J2-style) for an RGB monitor/scart before AD724 is trusted
...
| Third **ATF22V10** | Only if you want Compositor-style index logic instead of discrete/DIP-switch index (§5). |
| Scope / logic analyzer | Strongly recommended for first light |

### Explicitly **do not** buy for Tier A
...
If any of those fail, fix that layer before adding S1, VRAM, or a CPU (Tier C / full board).
...
4. **Keep the analog island short:** PROM → DAC resistors → AD724 R/G/B inputs → RCA. Separate digital switching noise from that path as much as breadboard allows.
5. **Power AD724 analog and digital** (`APOS`/`DPOS`, `AGND`/`DGND`) correctly; enable encode (`ENCD` = H).
6. **Set `STND`** for your region (H = NTSC, L = PAL) and match FSC frequency.
...
1. **Don’t hang cart flash, RAM, or MCU data pins on any shared bus.** Tier A has no bus fights by design—keep it that way.
...
4. **Don’t leave HC-style enables floating** if you add any 74xx later; for pure Tier A you simply omit them.
...
7. **Don’t skip decoupling** on a breadboard at 5 MHz—noise will look like “bad colors” or unlocked chroma.
...
For Tier A you do **not** need correct active/blanking pixel art yet—you need **stable** H and V timing that a TV will lock to. Approximate NTSC-ish blanking is enough for first light:
...
Exact Retr01 HBlank/VBlank equations live in the Beam X/Y JEDEC files when you implement the full design. For the lab, implement a **minimal** counter + sync pulse generator in the two PLDs (or one PLD + discrete logic if you are learning).
...
1. Wire six switches (or jumpers) to PROM A[5:0] with pull-downs (or pull-ups—pick one and be consistent).
...
2. **External 74HC logic:** e.g. take X[7:5] through a small PROM/GAL or hardwired map into 6 bits (still fine for a lab; not part of the final BOM).
3. **External tiny MCU (not Retr01 S1):** only as a temporary pattern generator driving A[5:0]; remove before calling the lab “PLD-pure.” Prefer A or B.1 for faithfulness.

During blanking, force index **0** (or hold last) so the DAC is quiet in sync regions if you care about cleaner composite.
...
Lets you prove PROM/DAC/AD724 independently of beam equations.
...
Convert each preview RGB to R3G3B2 for the burner, or use Studio’s `<stem>_prom.bin` (64 packed bytes) when you have a project export. Indices **0…15** darkest → **48…63** brightest.

**OTP note:** AT27C256R is one-time programmable in normal use. Verify the image on a burner that supports 27C256 VPP/VCC before committing, or use a UV-erasable equivalent only if your process allows it.
...
7. **Only then** plan Tier C (S1 + field SRAM), not before.
...
| **Tier C** | MCU-S1 + field SRAM (+ optional HC573) for VBlank pattern fill |
...
*Retr01 Tier A video lab — breadboard-only, video path validation.*  
*Aligned with repo docs as of the locked BOM / raster / color kit.*
