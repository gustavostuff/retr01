...
| Cycles per frame (~60 Hz) | ~29 800 | **~133 000** (~4.5×) |
| Graphics work in PRG | Heavy: software scrolling, sprite multiplexing, VBlank-only nametable updates, attribute tables | Minimal: hardware 2×2 camera window, interleaved VRAM, sprite line-buffer (1284), MAP streaming via `$FE93`, per-tile bank/attr already in hardware |
...
- **Solid physics & collision**: tile-based or soft-pixel, platforms, slopes, one-way platforms, multiple hitboxes per entity  
...
- **World & camera logic**: multi-screen seamless scrolling (already helped by the live 2×2 VRAM window), doors/warps, triggers, cutscene scripting, simple dialogue  
- **Game flow**: title → world select → up to 8 worlds × 32 screens, save points, quest flags, ending sequence  
- **Audio driver**: modest music + SFX bytecode that the ATmega328P mixes  

Classic NES 32 KB titles (Super Mario Bros., early Metroid/Zelda-style exploration, etc.) already delivered complete, memorable games under far tighter constraints.  
On Retr01 the same designs leave substantial headroom for richer AI, better physics, inventory systems, and multi-world structure while still fitting cleanly in the fixed 32 KB window.
...
32 KB of PRG on Retr01 is therefore enough for a complete, polished 8-bit action, platform or adventure game with multiple worlds and modern-feeling camera work.
