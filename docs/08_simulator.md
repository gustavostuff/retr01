...
`r01s_board_softboot_start_screen` is **not** the default. Set `R01S_SOFTBOOT=1` to poke VRAM/pals and advance `map_addr` without the CPU stream (debug / triage). Default = IC stream ownership.
