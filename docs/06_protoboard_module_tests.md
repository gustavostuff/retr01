...
- **W65C02S** (not NMOS 6502): pin **36 `BE` = high**, pin **`RDY` = high**, pins **1 `VPB`** and **5 `MLB`** NC. Full DIP-40 table: **§3.2**.
- Clock input is **`PHI2`**, not φ0/φ1 split like some older docs.
...
Each module below lists **which ICs** it uses. Pin names, datasheet links, and Retr01 wiring notes are in **§3**.
...
| **AS6C62256-55PCN** | DIP-28 | 32 KB SRAM (×3) | [Alliance AS6C62256 PDF](https://www.alliancememory.com/wp-content/uploads/pdf/datasheets/AS6C62256.pdf) |
...
| 4 | IRQB | **3.3 kΩ → +5 V**; GAL-TIM can pull low |
| 5 | MLB | **NC** |
| 6 | NMIB | **3.3 kΩ → +5 V**; GAL-TIM VBlank pulse |
...
| 35 | NC | — |
| 36 | BE | **3.3 kΩ → +5 V** (always enabled) |
| 37 | **PHI2** | **8 MHz oscillator out** |
| 38 | SOB | **3.3 kΩ → +5 V** |
| 39 | PHI2O | NC (optional) |
| 40 | RESB | HC14 output; RC + button per §4 |

Active levels: **`RESB`**, **`IRQB`**, **`NMIB`** are **low** active. **`RWB` high = read**.

### 3.3 AS6C62256 — DIP-28 (modules C, G, M)

One chip = **32 K × 8** (address **A0–A14**). Three chips on Retr01-A: system RAM, VRAM, line buffer.
...
| 21 | `/WE` | Write strobe (active **low**; qualify with PHI2 on VRAM) |
| 22–27 | A9, A8, A11–A14 | Address |
| 28 | VCC | +5 V + **100 nF** |

**System RAM (module C):** A0–A14 ← CPU A0–A14. I/O0–7 ↔ CPU D0–7. `/CE` ← `!A15`. `/OE` ← read cycle. `/WE` ← write cycle + `RWB`.

**VRAM (module G):** A0–A14 ← **4× 74HC157** mux (CPU latched addr vs PPU addr). I/O ↔ **74HC245** to CPU on CPU phase only. `/CE`, `/OE`, `/WE` qualified by GAL-PPU + PHI2.
...
Quad 2-input mux. **One IC = 4 bits.** VRAM needs **4× 157** for 15-bit address (+1 bit tied or expanded).
...
Pin numbers vary by fuse map — after programming, print a **pinout report** from your programmer and paste into your island notebook. [Microchip ATF22V10 PDF](https://ww1.microchip.com/downloads/en/DeviceDoc/ATF22V10-Datasheet-DS50002239D.pdf) § pin diagram.
...
| F | + AT28C64B |
| G | + AS6C62256, 4× HC157, HC245, HC573 |
| H | + 4× HC161, ATF22V10, HC688 |
| I | + HC573 (scroll), glue from §3.7 |
...
- **5.5 × 2.1 mm** barrel, center **+**, or bench supply direct.
- **Polyfuse** ~500 mA–1 A in series.
- Reverse-protection **diode** across 5 V (cathode to +).
- **100 µF** bulk + **100 nF** at the entry point.
- **LED + 1 kΩ** from +5 V to GND as power-on marker.

### CPU reset (`RESB`)

- **10 kΩ** pull-up `RESB` → +5 V.
- **10 µF** `RESB` → GND (slow POR).
- **74HC14** Schmitt: raw RC node → HC14 in → CPU `RESB`.
- Tact switch: RC node → GND for manual reset.

### CPU straps (W65C02S)

- **`RDY`**, **`BE`**, **`SOB`**: **3.3 kΩ** → +5 V.
- **`NMIB`**, **`IRQB`**: **3.3 kΩ** → +5 V.
- **`VPB`**, **`MLB`**: no connect.

### Pad switches (island E)

- Each switch: **10 kΩ** pull-up to +5 V, switch to GND (active low).
...
| 1284 | **20 MHz** HC-49 + **22 pF** ×2 to GND |
| 328P | **16 MHz** HC-49 + **22 pF** ×2 to GND |

100 nF on each oscillator/can VCC pin.

### Video DAC (island O, real silicon)

- **6-bit R-2R** per gun (1 kΩ / 2 kΩ), **75 Ω** series to output.
- Target ~**0.7 V** into 75 Ω; tune on bench.
- **CSYNC** negative.

### AVR ISP

- 6-pin header: MISO, MOSI, SCK, `/RESET`, VCC, GND.
- **10 kΩ** on AVR `/RESET` → +5 V.
...
2. Measure rail: **5.0 V ± 0.25 V** under a **100 Ω** dummy load.
...
**Pin reference:** W65C02S **§3.2** · AS6C62256 **§3.3** · ATF22V10 **§3.8** · cart flash **§3.11**
...
- **`PHI2`** → CPU clock; **`R/W`**, **`RESB`**, straps per §3.
...
**Pin reference:** HC573 **§3.4** · ATF22V10 **§3.8**
...
|------|------|
| AS6C62256 #2 | VRAM |
| 4× 74HC157 | 15-bit address mux (CPU vs “PPU side”) |
...
**Pin reference:** AS6C62256 **§3.3** · HC157 **§3.6** · HC245 **§3.5** · HC573 **§3.4**
...
- Missing 157 — need **4×** 157 for 15 bits (one 157 = 4 bits).
...
341×262 timing matches [`02_graphics_worlds_memory.md`](02_graphics_worlds_memory.md).
...
- Parallel flash (or 4× 512 KB DIP) on a **cart connector** or breadboard “cart.”
...
**Pin reference:** ATmega328P **§3.9** · [full PDF pinout](https://ww1.microchip.com/downloads/en/DeviceDoc/ATmega328P-DS-DS40002061A.pdf)
...
**Pin reference:** ATmega1284P **§3.9** · [full PDF pinout](https://ww1.microchip.com/downloads/en/DeviceDoc/40002047A.pdf)
...
- **2× 74HC157**: address = **`{display_half, X[7:0]}`** vs 1284 address.
...
256×240-ish stable image; **~15.7 kHz** horizontal rate class; no illegal bus contention.
...
| **§3 above** | Datasheet links + Retr01 pin wiring for every island IC |
