...
The entire app UI is drawn to a fixed **640x360** logical canvas.

| Setting | Value |
|---------|-------|
| Default window scale | **2x** integer (1280x720) |
...
Implementation note: distinguish **UI canvas** (640x360 layout space) from **game screen resolution** (128x120). The Screen cell shows a 128x120 edit viewport inside the larger UI.

### Cell manager (not floating windows)

Studio uses a **window-manager-style cell layout**: fixed regions ("cells") arranged on the 640x360 canvas. Cells are **not** free-floating mini-windows and are **not user-resizable** in v1.
...
**Purpose:** Show the four CHR **BG banks** (256 tiles each, **16x16** tile grid) for the current world.

| Feature | Phase 1 | Later |
|---------|---------|-------|
| Tabs | **4 tabs** (banks 0-3) | same |
| Grid | **256 cells** per tab (16x16), read-only display | optional direct tile edit |
| Source | Filled by **Generate bank** from Screen cell | import, duplicate, manual edit |

Each cell represents one **8x8** BG tile index (0-255) in that CHR bank.
...
**Purpose:** Paint one **16x15** nametable screen (128x120 px) at **8x8** tile resolution.
...
**Generate bank** scans the painted BG layer, deduplicates unique 8x8 patterns, packs them into the selected CHR BG bank (up to 256 tiles), and rewrites the screen's **nametable tile indices** to point at the packed tiles. Updates the BG banks cell (read-only grids).
...
- [ ] Default **2x** integer window scale
...
- [ ] Edit **one screen** for the selected world grid position (16x15 tiles, 128x120 px)
...
- Can paint at least one full 16x15 BG screen.
...
- **Unit tests** for CHR pack, project round-trip, and grid caps are green (see section 7).
