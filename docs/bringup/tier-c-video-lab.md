...
**SoT:** `docs/general/hardware.md`, `docs/general/video-graphics.md`, `docs/general/ic-comms-risks.md`, `docs/ic_behavior/AVR128DB28.md`, `docs/ic_behavior/AS6C62256.md`, `docs/ic_behavior/74HC573.md`.
...
| **Sprites** | 1-2 small figures (e.g. 2x2 or 2x3 of 8x8 tiles): stand + simple walk frames | **Field SRAM**, rebuilt (or page-flipped) in **VBlank** |

Practical approach:

1. Store a few **8x8** 2bpp tiles (or even 1-bit masks + fixed kit colors) in S1 flash/`.rodata`.
...
**Avoid for first firmware:** full 128x120 software framebuffer every frame, per-scanline sprite eval in HBlank, or streaming from a cart (no cart yet).
...
- AD is Hi-Z outside write windows. Field `/OE` (beam) and `/WE` (S1) stay mutually exclusive.
- Prefer **55 ns** SRAM. Field traces stay short on a breadboard as much as possible.
- Order is one static sprite, then animation, then a second sprite, then BG0 lines.
- AVR decoupling is heavy. UPDI only when a perfect picture is not required.

### Forbidden

- Field fill in HBlank or active display.
- Long SPI/I2C on S1 during the video windows that matter (nothing to talk to yet).
- MCU-M "just for OAM SPI" before field-direct blits work (that is Tier D).
- Host Play / Studio art pipeline expectations. This lab is hand-authored tiles in firmware.
- A second field SRAM as the long-term BG0 home.
