...
**Design SoT:** `docs/general/hardware.md`, `docs/general/video-graphics.md`, `docs/general/palette/`, `docs/general/memory.md`, `docs/general/ic-comms-risks.md`, `docs/general/software-api.md`, plus `docs/ic_behavior/`.
...
6. **Hardest remaining risks (not blockers to the plan).** MCU-M answering soft `$7Fxx` inside one PHI2 cycle without habitual `CPU_RDY`. VRAM half-cycle margin through 3x HC157 + PLD vs 55 ns SRAM. Full sprite field + early-VBlank OAM SPI both fitting before active display. Those are measurement gates at F, E, and D/C. They are not reasons to reorder tiers.
...
3. **Hi-Z and single-driver rules** on D and on S1 AD (`docs/general/ic-comms-risks.md`).
