...
| Offset | Field |
| --- | --- |
| `[0]` | Bit 0 = enable. Bits 4-7 = volume 0-15. Host mix honors volume on BGM 1-3 (wavetable) |
...
| Ch | Index | Mix | Role |
| --- | --- | --- | --- |
| 1 | 0 | Host wavetable | Lead. Instrument dropdown (Guitar / EGuitar / Piano / Flute) |
| 2 | 1 | Host wavetable | Harmony / arpeggio / counter-melody |
| 3 | 2 | Host wavetable | Bass or extra melody. Same four instruments |
...
A max-fill cart leaves ~**66.7 KB** for this blob (`memory.md`). That is roughly **24 minutes** of busy 5-channel BGM, or about **40 minutes** at a sparser 3-channel density. Unique sixteenth-note rows land closer to **13 minutes**.
...
ASCII tracker tokens with `#` encode as the enharmonic flat (C# is D-flat).
...
Packed PRG NMI ticks this tracker (`apps/sdk/r01_c/src/tracker.c`) and applies FD frames into the 8x4 window. That is the cart protocol. The PC speaker mixes that window (`r01_apu_mix.c`). It is not MCU-S2 PWM.
