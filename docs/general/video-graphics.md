...
Collision marks BG1 patterns by bank index and tile index. Palette and H/V flip do not affect solidity. Author code registers those patterns with `r01_solid_pattern_add` in `custom_logic.c`. The packed list lives in system RAM (`$0200`, copied from PRG `$8700` at boot). A BG1 cell is solid when its bank and tile match a marked pattern. See `software-api.md`.
