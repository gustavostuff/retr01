...
Collision marks BG1 patterns by bank index and tile index. Palette and H/V flip do not affect solidity. The list of solid patterns lives in system RAM (`$0200`, copied from PRG `$8700` at boot). A BG1 cell is solid when its bank and tile match a marked pattern. See `software-api.md`.
