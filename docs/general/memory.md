...
The `$8700` list is the Set Solid pattern pairs only. Collision samples BG1 nametable bank+tile against that list (plus optional `r01_solid_pattern_add` extras in RAM). Play reads the live 2x2 from RAM. Off-window cells read MAP (tile + attr only). llvm-mos C occupies `$C400-$FFF9`. Table growth into `$C400` fails the link.
