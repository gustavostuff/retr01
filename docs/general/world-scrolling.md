...
Author code sets wrap with `r01_bg0_set_wrap(ctx, wrap_x, wrap_y)` and clip with `r01_bg0_set_clip_to_bg1(ctx, enable)` in `custom_logic.c`. Studio packs those calls into the flag bits at cart export (same scan path as camera dead zone). `r01_game_set_mode(ctx, R01_GAME_MODE_PLATFORMER)` packs bit **4**. `r01_solid_pattern_add(ctx, bank, tile)` packs the solid-pattern list at PRG `$8700`.
