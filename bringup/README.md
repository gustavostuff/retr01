...
**When to use:** After Studio/Emu contracts and the design SoT are stable. These notes are late-phase hardware bring-up, not day-one software work.
...
6. **Hardest remaining risks (not blockers to the plan):** (a) MCU-M answering soft `$7Fxx` inside one PHI2 cycle without habitual `CPU_RDY`; (b) VRAM half-cycle margin through 3× HC157 + PLD vs 55 ns SRAM; (c) full sprite field + early-VBlank OAM SPI both fitting before active display. Those are measurement gates at F, E, and D/C — not reasons to reorder tiers.
...
**Skip:** M, 6502, cart, SPI OAM from outside S1. Do **not** treat a second SRAM as the design path for BG0 (lab shortcut only if explicitly temporary).
...
**Add:** PHI2 **8.000 MHz** canned oscillator (series ~33 Ω); AS6C62256 as interleaved VRAM; 3× 74HC157 (G never floating).  
...
1. **One new risk domain per tier** — do not combine cart + pads + 6502 in one jump.
...
When H’s demo is dull, the breadboard path has reached full console behavior with pads; PCB spin and software depth are separate tracks.
