...
**SoT:** `general_docs/memory.md`, `general_docs/hardware.md`, `general_docs/ic-comms-risks.md` (soft `$7Fxx` / `CPU_RDY`), `general_docs/software-api.md`, `ic_behavior/W65C02S.md`, `ic_behavior/AVR128DB28.md`.
...
Do not invent mappers or banking. Flat 32 KB PRG remains the contract.
...
- Bring up decode islands: RAM alone, soft alone, PRG alone, then combine.
- Keep OAM SPI early VBlank / `S1_RDY`.
- Document measured soft-cycle timing vs PHI2.

### Don’t

- Don’t add cart OE fights yet.
- Don’t poll pads (no S2).
- Don’t hold RDY across full OAM SPI or multi-ms work.
- Don’t mid-frame scroll/palette writes “because the bench looks fine.”
