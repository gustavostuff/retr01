...
How a Retr01 cart lays out maps. Graphics timing, CHR banks, palettes, and live VRAM are in [02_graphics_and_cartridge.md](02_graphics_and_cartridge.md). The MAP port wiring is in [08_memory_map.md](08_memory_map.md).

A **world** is a cart chapter: **4 BG banks + 4 sprite banks** plus a sparse atlas of screens. A cart has up to **8** worlds (0-7). `$FE30` world select picks which chapter's CHR the PPU is drawing. Software `world` in RAM should match when you change chapter.
...
Each stored screen also carries a **BG bank 0-3** in its MAP metadata. When software loads that screen into a live VRAM **nametable slot**, it copies both the nametable bytes **and** the screen's BG bank into that slot's **BG bank latch**.
...
CHR: **4 BG banks + 4 sprite banks per world**, 256 patterns each. That stays in [02_graphics_and_cartridge.md](02_graphics_and_cartridge.md).

## Example layouts

These are **examples**, not an enum. Any set of at most 64 grid positions on the 16x16 grid is legal. The screenshot is one roll of [`world_examples_generator/`](../world_examples_generator/). Grid sizes and screen counts change when you randomize.
...
- Directory **hit** on a **playfield** grid position: decompress that screen.
- Directory **hit** on a **parallax** grid position: treat as a miss for the camera (empty template). You cannot walk, warp, or seam-stream into it. `set_parallax` is the only loader that uses that payload.
...
**Streaming cue (software, not a PPU register):** when the camera is within **2 tiles (16 px)** of a seam, look up neighbor `(map_x±1, map_y)` or `(map_x, map_y±1)`. Hit: decompress into the incoming nametable slot and copy that screen's BG bank into the same slot's **BG bank latch**. Miss: empty template.
...
+-- world N
      ...
      directory[screen_count]:
          ...
          flags               ; bit0 = parallax, bits1-2 = BG bank 0..3, bits3-7 reserved
...
world_01:
    ...
    .incbin "cave.bin"
    .incbin "sky_a.bin"   ; flags bit0=1. H span=2 with sky_b, not enterable
    .incbin "sky_b.bin"
```
...
5. Copy that directory row's **BG bank bits** into the destination slot's BG bank latch.
6. Coords outside 0-15: optional lettered EMPTY debug fill.

Seam fill is the same lookup. A parallax neighbor is a miss (empty / blocked). `set_parallax` is a separate call: same directory lookup, but it decompresses into plane nametable slot 4 or 5 **and** copies the plane screen's BG bank into that slot's **BG bank latch**. See [02_graphics_and_cartridge.md](02_graphics_and_cartridge.md) section 8.
