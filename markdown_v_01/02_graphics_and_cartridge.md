...
| Tile size | 8x8 |
| Nametable | **32x30** tile indices (960 bytes) |
...
+-- World 0..7                      (up to 8)
    ...
    +-- Screens 0..63               (up to 64)
        +-- 32x30 nametable + per-tile BG attributes
...
- **BG palette is selected per tile** (not NES 2x2 attribute blocks).
...
|----------|----------------|
| 4 nametable slots x (960 tiles + 960 attrs), 2 KiB-aligned | 4 x 2 KiB = **8 KiB** |
...
Smooth pixel scrolling can show tiles from **up to four screens** at once (camera on a 2x2 seam). Hardware keeps a **2x2 nametable grid** (64x60 tiles addressable via scroll). A **~5-tile-thick** perimeter around the 32x30 viewport is the CPU's cue to stream the next strip from MAP-ROM into the slot that is scrolling into view - not an extra fifth nametable.
