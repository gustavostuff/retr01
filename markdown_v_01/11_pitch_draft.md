...
**8 worlds** on a sparse grid up to 16×16, at most 64 stored screens each. A screen is 32×30 tiles. Each world has **4 BG cells + 4 sprite cells**, 256 patterns each. A ~2 MB cart can hold on the order of **512** screens if you pack the atlas. **MAP** is a directory the CPU reads through `$FE90` (you do not mmap the whole world; you ask for a room and stream it).

**Raster IRQ** on a scanline compare (an interrupt on a line you pick). Split the screen, swap CHR cells, run a parallax band. No burned sprite-0 pixel.
