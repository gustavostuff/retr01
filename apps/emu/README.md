...
| **Host** | SDL. Pad map: P1 WASD+G/H, P2 arrows+,/. P1 X/Y edge = warp + fixed SFX. Host softsynth BGM (mix / 4). Not cart `$7F40` audio |
...
**Collision:** Host Play reads **cart MAP attrs** (`R01_ATTR_SOLID`). Player hitbox is the **current anim state's** AABB, origin-relative via that state's first drawable frame in the cart player anim blob when present. PRG collision stub at `$8500` is packed for future 6502 use. Host movement does not call it today.
