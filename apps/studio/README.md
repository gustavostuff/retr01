...
| **Entities** | Primary object authoring. Types with up to **4** states x **4** frames x **4** sprites. Modal: full-width **Name** / **State name**, **State**/**Frame** strips, larger compose canvas (fixed scale, no zoom). Right-click canvas: **Add sprite** (allocates into the world SPR bank, max 4/frame). Select/drag brings a sprite to front. **Paint** checkbox: LMB paints the topmost sprite only. **Space** toggles light part outlines. **Origin/hitbox** checkbox shows guides (auto-computed from the state sprite bounding-box center). Sidebar hover: name + type id. Right-click list: **Edit** / **Mark as player** / **Remove**. Soft caps and **boss** assemblies (optional BG body + multi-entity attachments): [`docs/video-graphics.md`](../../docs/video-graphics.md). Cart packs locked EntityDef catalog (docs/software-api.md) |
...
| Undo / redo | **Ctrl+Z** / **Ctrl+Y** (or **Ctrl+Shift+Z**). Covers map paint strokes, flood fill, entity compose sprite paint, add tile/sprite/entity/metasprite/metatile, place/remove instance, create/remove/paste screen |
