...
| **Ctrl+C / Ctrl+V** | **Screen preview** (BG / Both): copy / paste the tile selection (multi-tile stamp). Paste at the tile under the cursor, or at the selection top-left if the cursor is off-preview. **World map:** copy / paste an entire selected screen (tiles+attrs) on BG0 or BG1. Paste creates the slot if empty |
