...
| **Collision** | Current anim-state hitbox (origin-relative via that state's first drawable frame) vs `R01_ATTR_SOLID` on cart MAP attrs (not PRG collision stub) |
...
| Compose | Each frame's sprite group is placed in the middle of the **32x32** authoring canvas. Draw origin sits on that group's bounding-box center. The default **8x8** hitbox is centered on the first drawable frame's group and stored on the **state**. Play pose is origin-relative per frame. Collision is origin-relative to the state's first drawable frame |
