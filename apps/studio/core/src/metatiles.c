...
int r01_world_metatile_add(R01Project *p) {
    R01MetatileDef *mt;
    if (!p || p->metatile_count >= R01_MAX_METATILES) {
        return -1;
    }
    mt = &p->metatiles[p->metatile_count];
    r01_metatile_init(mt, "Metatile");
    p->metatile_count++;
    return p->metatile_count - 1;
}

int r01_world_metatile_remove(R01Project *p, int idx) {
    int i;
    if (!p || idx < 0 || idx >= p->metatile_count) {
        return -1;
    }
    for (i = idx; i < p->metatile_count - 1; i++) {
        p->metatiles[i] = p->metatiles[i + 1];
    }
    p->metatile_count--;
    memset(&p->metatiles[p->metatile_count], 0, sizeof(p->metatiles[0]));
    return 0;
}
