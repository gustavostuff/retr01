...
TEST_MAIN() {
    ...
    EXPECT(r01_chr_find_spr_bank_space(p) == 0, "prefer bank 0");
    bank = 0;
    ...
    tile[0] = 0x81;
    tile[8] = 0x42;
    EXPECT(r01_chr_write_spr_tile(p, bank, id, tile) == 0, "write spr tile");
    cat = r01_world_sprite_add(p, bank, id, 2);
    EXPECT(cat == 0, "catalog add");
    EXPECT(p->sprite_count == 1, "sprite count");
    EXPECT(p->sprites[0].pal == 2, "default pal");

    id = r01_chr_alloc_spr_tile(p, bank);
    EXPECT(id == 1, "second alloc is contiguous (tile 1 ok in authoring)");

    EXPECT(r01_world_sprite_set_pal(p, 0, 3) == 0, "set pal");
    EXPECT(p->sprites[0].pal == 3, "pal updated");

    /* Fill bank 0 to force bank 1. */
    while (r01_chr_alloc_spr_tile(p, 0) >= 0) {
    }
    EXPECT(r01_chr_find_spr_bank_space(p) == 1, "overflow to bank 1");
    ...
    EXPECT(p2->sprite_count == 1, "catalog roundtrip count");
    EXPECT(p2->sprites[0].bank == 0, "catalog bank");
