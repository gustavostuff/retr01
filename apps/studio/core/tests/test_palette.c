...
TEST_MAIN() {
...

    {
        ...
        id_blank = r01_chr_alloc_tile(p, 0);
        id_fill = r01_chr_alloc_tile(p, 0);
        EXPECT(id_blank >= 0 && id_fill >= 0, "alloc blank+fill tiles");
        r01_chr_write_tile(p, 0, id_blank, tblank);
        r01_chr_write_tile(p, 0, id_fill, tfill);
        ...
        EXPECT(r01_screen_pixel_color(p, bg1, 0, 0) == 0, "BG1 transparent");
        EXPECT(r01_screen_pixel_color(p, &bg0, 0, 0) == 1, "BG0 opaque");
