...
TEST_MAIN() {
    ...
    /* v5 sprite catalog roundtrip */
    {
        ...
        int id = r01_chr_alloc_spr_tile(p, 0);
        memset(tile, 0xA5, sizeof(tile));
        EXPECT(id >= 0, "alloc spr");
        EXPECT(r01_chr_write_spr_tile(p, 0, id, tile) == 0, "write spr");
        EXPECT(r01_world_sprite_add(p, 0, id, 1) == 0, "add sprite");
        ...
        EXPECT(p2->sprite_count == 1, "sprite count v5");
        EXPECT(p2->sprites[0].pal == 1, "sprite pal v5");
    }
