...
static int setup_board(R01sApp *app, int argc, char **argv) {
    ...
    r01s_app_mount_builder(app);
    {
        ...
        /* Pin stream: palette + start screen through $7Fxx, then Host Play. */
        r01s_board_arm_ic_boot(&g_board);
        fprintf(stderr, "cart: IC boot stream armed (palette + MAP via $7Fxx)\n");
