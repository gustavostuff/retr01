...
void r01s_app_mount_builder(R01sApp *app) {
    ...
    for (i = 0; i < b->mount_count; i++) {
        ...
        /* Power / OSC stay on the board netlist but are not UI chips. */
...
void r01s_app_frame(R01sApp *app) {
    ...
    if (catching_up) {
        ...
        if (board) {
            ...
            r01s_board_set_wire_mode(board, app->ui.wire_mode == R01S_WIRE_MANUAL
                                                ? R01S_BOARD_WIRE_MANUAL
                                                : R01S_BOARD_WIRE_LIVE);
            ...
            if (board->play.enabled && board->apu_impl.apu) {
                ...
                r01s_board_host_apu_fe4x_push(board);
            }
        }
        if (group) {
            if (group->running) {
