...
static uint8_t orient_from_json(const char *s) {
    if (!s || !s[0]) {
        return (uint8_t)R01S_ORIENT_0;
    }
    if (s[0] == 'V' || s[0] == 'v') {
        return (uint8_t)R01S_ORIENT_90;
    }
    if (s[0] == 'H' || s[0] == 'h') {
        return (uint8_t)R01S_ORIENT_0;
    }
    ...
    if (strcmp(s, "180") == 0) {
        return (uint8_t)R01S_ORIENT_180;
    }
    if (strcmp(s, "270") == 0) {
        return (uint8_t)R01S_ORIENT_270;
    }
    if (strcmp(s, "1") == 0) {
        return (uint8_t)R01S_ORIENT_90;
    }
...
int r01s_ui_layout_save(R01sUi *ui) {
    ...
    fprintf(f, "  \"wave_monitor_x\": %d,\n", ui->wave_monitor_x);
    fprintf(f, "  \"wave_monitor_y\": %d,\n", ui->wave_monitor_y);
    ...
    for (i = 0; i < ui->chip_count; i++) {
        ...
        if (ui->layout_compact) {
            ...
            rx = ui->save_chip_x[i];
            ry = ui->save_chip_y[i];
            orient_s = orient_to_json((R01sPkgOrient)ui->save_chip_orient[i]);
        } else {
            ...
            orient_s = orient_to_json(e->orient);
        }
        if (!first) {
            fprintf(f, ",\n");
        }
    ...
    if (ui->compact_saved) {
        for (i = 0; i < ui->chip_count; i++) {
            ...
            fprintf(f, "    {\"id\": \"%s\", \"x\": %d, \"y\": %d, \"orient\": \"%s\", \"z\": %d}", id,
                    ...
                    orient_to_json((R01sPkgOrient)ui->compact_chip_orient[i]),
...
int r01s_ui_layout_load(R01sUi *ui) {
    ...
    int wave_monitor_x = R01S_UI_WAVE_MONITOR_DEFAULT_X;
    int wave_monitor_y = ui_wave_monitor_default_y();
    char mode[16];
    const char *section;
    const char *obj;
    const char *end;
    ...
    json_int_after(buf, "\"wave_monitor_x\"", &wave_monitor_x);
    json_int_after(buf, "\"wave_monitor_y\"", &wave_monitor_y);
    ...
    if (section) {
        ...
        /* v3: drop empty soft-$7Fxx island D (old index 3). */
        if (file_version < 3) {
        ...
        while (obj && (!stop || obj < stop)) {
            ...
            if (json_string_after(slice, "\"id\"", id, sizeof(id))) {
                ...
                if (ci >= 0) {
                    ...
                    ui->save_chip_orient[ci] = orient_from_json(orient);
        ...
        while (obj) {
            ...
            if (json_string_after(slice, "\"id\"", id, sizeof(id))) {
                ...
                if (ci >= 0) {
                    ...
                    ui->compact_chip_orient[ci] = orient_from_json(orient);
                    ui->compact_saved = 1;
                }
            }
            obj = json_next_object(end);
    ...
    ui->wave_monitor_x = wave_monitor_x;
    ui->wave_monitor_y = wave_monitor_y;
    /* UI is compact-only. Islands mode remains in saved JSON for tests / bring-up. */
