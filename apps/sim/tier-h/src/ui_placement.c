...
static void clamp_chip_to_board(R01sEntity *e) {
    ...
    max_x = R01S_BOARD_W + over - e->body_w;
    max_y = R01S_BOARD_H + over - e->body_h;
    if (max_x < min_x) {
        max_x = min_x;
    }
    if (max_y < min_y) {
...
static void clamp_chip_in_island(R01sUi *ui, R01sEntity *e, int island_index) {
    ...
    if (max_x < min_x) {
        int need = e->body_w + 2 * R01S_CHIP_PIN_OUT + 2 * R01S_ISLAND_PAD_X;
    ...
    if (max_y < min_y) {
        int need = e->body_h + R01S_ISLAND_PAD_TOP + R01S_ISLAND_PAD_BOTTOM;
...
static int pin_tip_hits_hole(const R01sBreadboard *bb, int tip_x, int tip_y, R01sPbHole *out) {
    R01sPbHole h;
    int hx, hy;
    if (!bb || !r01s_breadboard_hit_hole(bb, tip_x, tip_y, &h)) {
        return 0;
    }
    r01s_breadboard_hole_world(bb, h, &hx, &hy);
    if (hx != tip_x || hy != tip_y) {
        /* Near a hole but not centered: still a snap candidate via delta. */
    }
    if (out) {
        *out = h;
    }
...
static int entity_tip_board(const R01sEntity *e, int pin_num, int *tx, int *ty) {
    if (!e) {
        return 0;
    }
    if (e->visual == R01S_ENTITY_VIS_PASSIVE) {
        return r01s_passive_tip_board((const R01sPassive *)(const void *)e, pin_num, tx, ty);
    }
    if (e->visual == R01S_ENTITY_VIS_IC) {
        return ui_chip_pin_tip_board(e, pin_num, tx, ty);
    }
    return 0;
}

static int count_pins_on_holes_after_delta(const R01sUi *ui, const R01sEntity *e, int dx, int dy) {
    int n;
    int count = 0;
    int bi;
    int pin_hi;
    ...
    if (e->visual == R01S_ENTITY_VIS_PASSIVE) {
        pin_hi = 2;
    } else if (e->visual == R01S_ENTITY_VIS_IC) {
        pin_hi = e->dip_pins > 0 ? e->dip_pins : e->pin_count;
    } else {
        return 0;
    }
    for (n = 1; n <= pin_hi; n++) {
        ...
        for (bi = 0; bi < ui->chip_count; bi++) {
            ...
            R01sPbHole h;
            int hx, hy;
            if (!be || be->visual != R01S_ENTITY_VIS_BREADBOARD || be == e) {
                continue;
            }
            if (!r01s_breadboard_hit_hole((const R01sBreadboard *)(const void *)be, tx, ty, &h)) {
                continue;
            }
            r01s_breadboard_hole_world((const R01sBreadboard *)(const void *)be, h, &hx, &hy);
            if (hx == tx && hy == ty) {
                count++;
                break;
            }
...
int ui_chip_snap_to_breadboard(R01sUi *ui, int chip_i) {
    ...
    for (n = 1; n <= pin_hi; n++) {
        ...
        for (bi = 0; bi < ui->chip_count; bi++) {
            ...
            if (!pin_tip_hits_hole((const R01sBreadboard *)(const void *)be, tx, ty, &h)) {
                continue;
            }
            r01s_breadboard_hole_world((const R01sBreadboard *)(const void *)be, h, &hx, &hy);
            dx = hx - tx;
            dy = hy - ty;
            count = count_pins_on_holes_after_delta(ui, e, dx, dy);
            if (count > best_count) {
                best_count = count;
                best_dx = dx;
                best_dy = dy;
    ...
    if (e->visual == R01S_ENTITY_VIS_PASSIVE) {
        R01sPassive *p = (R01sPassive *)(void *)e;
        r01s_passive_set_pivot(p, p->pivot_x + best_dx, p->pivot_y + best_dy);
    } else {
        r01s_entity_place(e, e->board_x + best_dx, e->board_y + best_dy);
    }
...
void ui_sel_snap_to_breadboard(R01sUi *ui) {
    ...
    if (ui->drag_chip >= 0) {
        ...
        ui_chip_snap_to_breadboard(ui, ui->selected);
    }
}

void move_chip_drag(R01sUi *ui, int chip_i, int board_mx, int board_my) {
    R01sEntity *e = ui->chips[chip_i];
    int nx = board_mx - ui->drag_grab_bx;
    int ny = board_my - ui->drag_grab_by;
    if (!e) {
        return;
    }
    if (e->visual == R01S_ENTITY_VIS_PASSIVE) {
...
void ui_begin_sel_drag(R01sUi *ui, int board_mx, int board_my) {
    ...
    for (i = 0; i < ui->chip_count; i++) {
        ...
        if (e && e->visual == R01S_ENTITY_VIS_PASSIVE) {
            const R01sPassive *p = (const R01sPassive *)(const void *)e;
            ui->sel_start_x[i] = p->pivot_x;
            ui->sel_start_y[i] = p->pivot_y;
        } else {
...
void move_selection_drag(R01sUi *ui, int board_mx, int board_my) {
    ...
    dx = board_mx - ui->sel_drag_ox;
    dy = board_my - ui->sel_drag_oy;
    ...
    for (i = 0; i < ui->chip_count; i++) {
        ...
        any = 1;
        start_x = ui->sel_start_x[i];
        start_y = ui->sel_start_y[i];
        body_w = e->body_w;
        body_h = e->body_h;
        if (e->visual == R01S_ENTITY_VIS_PASSIVE) {
            /* sel_start is pivot. Approximate clamp with current AABB size about pivot. */
            start_x = e->board_x + (ui->sel_start_x[i] - ((const R01sPassive *)(const void *)e)->pivot_x);
            start_y = e->board_y + (ui->sel_start_y[i] - ((const R01sPassive *)(const void *)e)->pivot_y);
        }
        min_x = -R01S_BOARD_W / 2;
        min_y = -R01S_BOARD_W / 2;
        max_x = R01S_BOARD_W + R01S_BOARD_W / 2 - body_w;
        max_y = R01S_BOARD_H + R01S_BOARD_W / 2 - body_h;
        if (max_x < min_x) {
            max_x = min_x;
        }
        if (max_y < min_y) {
        ...
        if (min_x - start_x > dx_lo) {
            dx_lo = min_x - start_x;
        }
        if (max_x - start_x < dx_hi) {
            dx_hi = max_x - start_x;
        }
        if (min_y - start_y > dy_lo) {
            dy_lo = min_y - start_y;
        }
        if (max_y - start_y < dy_hi) {
            dy_hi = max_y - start_y;
        }
    }
    if (!any) {
    ...
    for (i = 0; i < ui->chip_count; i++) {
        ...
        if (e->visual == R01S_ENTITY_VIS_PASSIVE) {
            r01s_passive_set_pivot((R01sPassive *)(void *)e, ui->sel_start_x[i] + dx, ui->sel_start_y[i] + dy);
        } else {
...
static void chip_pack_footprint(const R01sEntity *e, int *pw, int *ph) {
    ...
    if (e->visual == R01S_ENTITY_VIS_BREADBOARD) {
        *pw = e->body_w > 0 ? e->body_w : 1;
        *ph = e->body_h > 0 ? e->body_h : 1;
        return;
    }
    /* Pin stubs stick out of the long sides of a DIP (H: L/R, V: T/B). */
    if (e->visual == R01S_ENTITY_VIS_IC && !r01s_orient_is_horiz(e->orient)) {
        *pw = e->body_w;
...
void ui_save_compact_layout(R01sUi *ui) {
    int i;
    if (!ui) {
        return;
    }
...
void ui_restore_compact_layout(R01sUi *ui) {
    int i;
...
void ui_apply_compact_layout(R01sUi *ui) {
    ...
    for (i = 0; i < ui->chip_count; i++) {
        const R01sEntity *e = ui->chips[i];
        if (!e || e->visual == R01S_ENTITY_VIS_NONE || e->visual == R01S_ENTITY_VIS_PASSIVE ||
            e->body_w <= 0 || e->body_h <= 0) {
            continue;
        }
        items[n].idx = i;
...
void compact_btn_rect(const R01sUi *ui, SDL_Rect *rc) {
    /* Islands mode removed from UI. Rect kept for layout save helpers only. */
    (void)ui;
    rc->x = R01S_LOGIC_W - R01S_UI_UNIT;
    rc->y = R01S_UI_UNIT / 2;
    rc->w = 0;
    rc->h = R01S_UI_UNIT * 2;
}
...
void wire_mode_btn_rect(const R01sUi *ui, SDL_Rect *rc) {
    SDL_Rect ibtn;
    const char *label = (ui && ui->wire_mode == R01S_WIRE_MANUAL) ? "MANUAL" : "LIVE";
    int tw = font_text_width(label) + R01S_UI_UNIT * 2;
    input_mode_btn_rect(ui, &ibtn);
    rc->w = tw;
    rc->h = ibtn.h;
    rc->y = ibtn.y;
    rc->x = ibtn.x - tw - R01S_UI_UNIT;
}
