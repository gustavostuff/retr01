...
int main(void) {
    ...
    expect_true(r01s_island_group_at(group, R01S_ISLAND_CART) != NULL &&
                    r01s_island_group_at(group, R01S_ISLAND_CART)->entity_count == 0,
                "console cart socket island empty");
    ...
    for (i = 0; i < 250000; i++) {
        ...
        if (b->health_saw_apu || r01s_avr128db28_s2_pwm_edges(&b->mcu_s2) >= 2) {
            saw_apu = 1;
        }
        if (b->health_saw_oam || (r01s_avr128db28_s1_oam_peek(&b->mcu_s1, 0) == 0x10 &&
                                  r01s_avr128db28_s1_alive(&b->mcu_s1))) {
            saw_oam = 1;
        }
        if (b->health_saw_linebuf || (b->field_saw_mcu && b->field_saw_beam)) {
            saw_linebuf = 1;
        }
    ...
    expect_true(saw_latch, "STA $7F02 hit soft $7Fxx");
    expect_true(saw_vram, "STA $7F12 wrote $AA to VRAM[0]");
    expect_true(saw_vram_read, "LDA $7F12 read VRAM back into A");
    expect_true(saw_pad, "LDA $7F60 read pads (MCU-S2 island K)");
    expect_true(saw_beam_hblank, "island H HBlank");
    expect_true(saw_beam_line, "island H advanced past line 0");
    expect_true(saw_raster_hit, "beam-Y PLD EQ# on Y vs $7F04");
    ...
    expect_true(saw_map, "island J MAP $7F93 read cart magic r");
    expect_true(saw_apu, "island K APU PWM tone edges");
    expect_true(saw_oam, "island L OAM $7F21 readback + clk");
    expect_true(saw_linebuf, "island L field SRAM + S1 path");
    expect_true(saw_sprites, "MCU-S1 OAM fill -> VBlank sprite field");
    ...
    expect_true(r01s_avr128db28_s1_oam_peek(&b->mcu_s1, 0) == 0x10, "OAM Y=$10");
    expect_true(r01s_avr128db28_s1_oam_peek(&b->mcu_s1, 1) == 0x01, "OAM tile=$01");
    expect_true(r01s_avr128db28_s1_oam_peek(&b->mcu_s1, 3) == 0x20, "OAM X=$20");
    expect_true(b->field_saw_mcu, "field MCU path");
    expect_true(b->field_saw_beam, "field beam path");
    ...
    {
        const char *paths[] = {R01_TEST_CART, NULL};
        int pi;
        int loaded = 0;
        for (pi = 0; paths[pi]; pi++) {
            if (r01s_board_load_cart(b, paths[pi]) == 0) {
                ...
                expect_true(b->cart_off_chr != 0, "global CHR base from cart");
                expect_true(b->cart_off_map_screen0 != 0, "world0 start-screen MAP payload");
                break;
            }
        }
        expect_true(loaded, "example cart found");
    }
