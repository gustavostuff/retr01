...
On startup (and after Ctrl+R reset), the sim must get the bring-up **palette + start-screen MAP** into VRAM before the LCD hold lifts. That is ~12k island steps of the IC `$7F93`->`$7F12` stream (`r01s_board_catchup_bringup` / yielding worker in `app.c`).
...
3. Cart PRG (or synthetic bring-up overlay if no cart file) runs the MAP/pal stream: LDA `$7F93` / STA `$7F12` (and palette path) until `map_addr >= cart_off_map_screen0 + 480` and VRAM[0] matches flash.
