...
static void board_eval_chips(R01aBoard *b) {
    ...
    ns_entity_eval(r01a_avr128db28_s1_entity(&b->mcu_s1));
    ns_entity_eval(r01a_sn74hc573_entity(&b->field_latch));
    ns_entity_eval(r01a_as6c62256_entity(&b->field_sram));
    ns_entity_eval(r01a_atf22v10_entity(&b->compositor));
...
static void board_bind_rails(R01aBoard *b) {
    ...
    drive_vdd(r01a_atf22v10_entity(&b->compositor), "VCC", vdd);
    ...
    drive_vdd(r01a_as6c62256_entity(&b->field_sram), "CE#", NS_LVL_L);
...
static void board_wire_compositor(R01aBoard *b) {
    NsEntity *bx = r01a_atf22v10_entity(&b->beam_x);
    NsEntity *by = r01a_atf22v10_entity(&b->beam_y);
    NsEntity *comp = r01a_atf22v10_entity(&b->compositor);
    NsEntity *prom = r01a_at27c256r_entity(&b->prom);
    int i;
    char iname[8];
    char aname[4];
    drive_copy(comp, "CLK", r01a_osc_dot_entity(&b->osc_dot), "DOT");
    drive_copy(comp, "HBLANK", bx, "HBLANK");
    drive_copy(comp, "X5", bx, "X5");
    drive_copy(comp, "X6", bx, "X6");
    drive_copy(comp, "X7", bx, "X7");
    drive_copy(comp, "VBLANK", by, "VBLANK");
    drive_copy(comp, "Y5", by, "Y5");
    drive_copy(comp, "Y6", by, "Y6");
    drive_copy(comp, "Y7", by, "Y7");
    for (i = 0; i < 6; i++) {
        snprintf(iname, sizeof(iname), "INDEX%d", i);
        snprintf(aname, sizeof(aname), "A%d", i);
        drive_copy(prom, aname, comp, iname);
    }
}

static void board_on_vblank(R01aBoard *b) {
    int ly;
    uint32_t frame;
    ns_video_sink_on_vblank(&b->sink);
    r01a_avr128db28_s1_bump_frame(&b->mcu_s1);
    frame = r01a_avr128db28_s1_frame(&b->mcu_s1);
    b->bg0_ping ^= 1;
    r01c_s1_vblank_field(&b->field_sram, frame);
    for (ly = 0; ly < R01A_FIELD_H; ly++) {
        r01c_s1_hblank_bg0_line(&b->field_sram, ly, b->bg0_ping);
    }
    r01a_atf22v10_bind_field(&b->compositor, r01a_field_mem(b), b->bg0_ping);
}

static void board_plot(R01aBoard *b) {
    int x = r01a_atf22v10_x(&b->beam_x);
    int y = r01a_atf22v10_y(&b->beam_y);
    int vblank = r01a_atf22v10_vblank(&b->beam_y);
    int vis;
    uint8_t idx;

    if (vblank && !b->prev_vblank) {
        board_on_vblank(b);
    }
    b->prev_vblank = vblank;
    b->prev_y = y;
    ...
    idx = r01a_atf22v10_index(&b->compositor);
...
static void board_settle(R01aBoard *b) {
    ...
    board_wire_compositor(b);
    {
        int lx = 0;
        int ly = 0;
        if (ns_rgbs_beam_to_logical(ns_video_sink_scale_2x(&b->sink), r01a_atf22v10_x(&b->beam_x),
                                    r01a_atf22v10_y(&b->beam_y), &lx, &ly)) {
            r01a_atf22v10_set_beam_dot(&b->compositor, lx, ly);
        } else {
            r01a_atf22v10_set_beam_dot(&b->compositor, -1, -1);
        }
    }
    r01a_atf22v10_bind_field(&b->compositor, r01a_field_mem(b), b->bg0_ping);
    drive_copy(r01a_avr128db28_s1_entity(&b->mcu_s1), "VBL", r01a_atf22v10_entity(&b->beam_y), "VBLANK");
    for (p = 0; p < R01A_SETTLE_PASSES; p++) {
        board_eval_chips(b);
    }
}
...
void r01a_board_step(R01aBoard *board) {
    ...
    if (board->wire_mode == R01A_WIRE_MANUAL) {
        ...
        board_bb_route(board);
        {
            int lx = 0;
            int ly = 0;
            if (ns_rgbs_beam_to_logical(ns_video_sink_scale_2x(&board->sink),
                                        r01a_atf22v10_x(&board->beam_x), r01a_atf22v10_y(&board->beam_y),
                                        &lx, &ly)) {
                r01a_atf22v10_set_beam_dot(&board->compositor, lx, ly);
            } else {
                r01a_atf22v10_set_beam_dot(&board->compositor, -1, -1);
            }
        }
        ns_entity_tick(r01a_avr128db28_s1_entity(&board->mcu_s1));
        ns_entity_tick(r01a_atf22v10_entity(&board->compositor));
        board_settle(board);
        board_plot(board);
        return;
    }
    ...
    ns_entity_tick(r01a_atf22v10_entity(&board->beam_y));
    board_wire_compositor(board);
    {
        int lx = 0;
        int ly = 0;
        if (ns_rgbs_beam_to_logical(ns_video_sink_scale_2x(&board->sink), r01a_atf22v10_x(&board->beam_x),
                                    r01a_atf22v10_y(&board->beam_y), &lx, &ly)) {
            r01a_atf22v10_set_beam_dot(&board->compositor, lx, ly);
        } else {
            r01a_atf22v10_set_beam_dot(&board->compositor, -1, -1);
        }
...
static void island_video_init(NsIsland *island) {
    ...
    r01a_atf22v10_init(&b->compositor, "UPLDC", R01A_PLD_COMPOSITOR);
    r01a_avr128db28_s1_init(&b->mcu_s1, "US1");
    r01a_sn74hc573_init(&b->field_latch, "U573");
    r01a_as6c62256_init(&b->field_sram, "U41");
    r01a_at27c256r_init(&b->prom, "U24");
    r01a_atf22v10_bind_field(&b->compositor, r01a_field_mem(b), 0);
...
static void group_reset(NsIslandGroup *group) {
    ...
    ns_entity_reset(r01a_atf22v10_entity(&b->compositor));
    ns_entity_reset(r01a_avr128db28_s1_entity(&b->mcu_s1));
    ns_entity_reset(r01a_sn74hc573_entity(&b->field_latch));
    ns_entity_reset(r01a_as6c62256_entity(&b->field_sram));
...
static void group_status(NsIslandGroup *group, char *buf, size_t buf_len) {
    ...
    snprintf(buf, buf_len, "X=%d Y=%d HB=%d VB=%d idx=%u", r01a_atf22v10_x(&b->beam_x),
             ...
             r01a_atf22v10_vblank(&b->beam_y), (unsigned)r01a_atf22v10_index(&b->compositor));
}
...
const uint8_t *r01a_field_mem(const R01aBoard *board) {
    if (!board || !board->field_sram.mem) {
        return NULL;
    }
    return board->field_sram.mem;
}
