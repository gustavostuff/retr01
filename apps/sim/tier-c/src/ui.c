...
#endif
#ifndef R01A_TIER_B_LAYOUT
#define R01A_TIER_B_LAYOUT "../tier-b/ui_layout.json"
#endif
...
static void pin_net_build(R01aUi *ui) {
...

    {
        ...
        NsEntity *u573 = ui_ent(ui, "U573");
        NsEntity *u41 = ui_ent(ui, "U41");
        char s1ad[8];
        char dn[4];
        char qn[4];
        char an[4];
        if (s1 && u573 && u41) {
            pin_net_link(y2, "VDD", s1, "VCC");
            pin_net_link(y2, "GND", s1, "GND");
            pin_net_link(y2, "VDD", u573, "VCC");
            pin_net_link(y2, "GND", u573, "GND");
            pin_net_link(y2, "VDD", u573, "OE#");
            pin_net_link(y2, "VDD", u41, "VCC");
            pin_net_link(y2, "GND", u41, "VSS");
            pin_net_link(y2, "GND", u41, "CE#");
            pin_net_link(y2, "GND", u41, "OE#");
            pin_net_link(by, "VBLANK", s1, "VBL");
            pin_net_link(s1, "ALE", u573, "LE");
            pin_net_link(s1, "/WE", u41, "WE#");
            for (i = 0; i < 8; i++) {
                snprintf(s1ad, sizeof(s1ad), "AD%d", i);
                snprintf(dn, sizeof(dn), "D%d", i);
                snprintf(qn, sizeof(qn), "Q%d", i);
                snprintf(an, sizeof(an), "A%d", i);
                pin_net_link(s1, s1ad, u573, dn);
                pin_net_link(u573, qn, u41, an);
            }
            pin_net_link(s1, "A8", u41, "A8");
            pin_net_link(s1, "A9", u41, "A9");
            pin_net_link(s1, "A10", u41, "A10");
            pin_net_link(s1, "A11", u41, "A11");
            pin_net_link(s1, "A12", u41, "A12");
            pin_net_link(s1, "A13", u41, "A13");
            pin_net_link(s1, "A14", u41, "A14");
        }
    }
}
...
int r01a_ui_run(R01aBoard *board) {
    ...
    ui.win = SDL_CreateWindow("Retr01 Tier C", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    if (r01a_layout_load(R01A_LAYOUT_FILE, board, &ui.pan_x, &ui.pan_y, &ui.zoom, &ui.air_always) != 0) {
        /* First launch: inherit Tier B layout (Tier A wiring + Compositor). */
        (void)r01a_layout_load(R01A_TIER_B_LAYOUT, board, &ui.pan_x, &ui.pan_y, &ui.zoom, &ui.air_always);
    }
    if (ui.zoom < 1) {
        ui.zoom = 1;
    }
