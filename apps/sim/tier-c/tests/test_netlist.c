...
int main(void) {
    ...
    {
        NsEntity *us1 = r01a_avr128db28_s1_entity(&board.mcu_s1);
        NsPbHole s1_h = {30, NS_PB_LANE_F};
        place_pin_on_hole(us1, 1, &board.breadboard, s1_h);
        n = r01a_netlist_check(&board, iss, R01A_NET_ISSUE_MAX);
        expect_true(!has_kind(iss, n, R01A_NET_SHORT_DATA) && !has_kind(iss, n, R01A_NET_SHORT_CHIP),
                    "seated US1 alone is not intra-chip shorted in Manual");
    }
