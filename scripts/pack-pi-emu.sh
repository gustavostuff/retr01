...
Logical framebuffer is **256x240**. The Pi binary has **no debug window**. `retr01.sh` starts at **1x** integer (256x240 centered in desktop-fullscreen, CRT / RGB-Pi OS4 Ports). Home / Guide on a pad opens Reset, Quit, and 1x/2x, and mutes the speaker while that overlay is open.
...
RGB-Pi OS4 Ports lists `.sh` files from USB `ports/`. `retr01.sh` launches that binary and cart at 1x integer (256x240 centered), 48 kHz / 2048-sample audio. The Pi CMake build defines `R01E_NO_DEBUG` so the debug window is never created.
...
Gamepad Home / Guide opens Reset / Quit / 1x-2x.
